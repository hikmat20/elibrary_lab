	...
	<div class="modal-dialog modal-dialog-centered modal-lg">
		<div class="modal-content">
			...
			<div style="padding:20px;">
				<table id="std-table" class="table table-bordered table-hover">
					<thead>
						<tr>
					...
					<tbody>
						<?php foreach($standards as $dt): ?>
							<tr>
								<td class="text-center">
									<input type="checkbox" class="row-check std-check"  value="<?= $dt->id; ?>" />
								</td>
								<td><?= $dt->name; ?></td>
							</tr>
						<?php endforeach; ?>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('click', '.save', function(e) {
			...
			var stdChecks = [];
			// console.log('test');
			$('.std-check:checked').each(function() {
				stdChecks.push($(this).val());
			});
			...
			if (c <= 0) {
				...
				formdata.append('stdChecks', JSON.stringify(stdChecks));
				let btn = $(this)
				$.ajax({
