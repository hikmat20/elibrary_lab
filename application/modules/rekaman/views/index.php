								...
								<?php else : ?>
										...
										<table class="table table-hover datatable" style="margin:0px 0px !important;">
											...
											<tbody>
												<?php foreach ($details as $dtl) : ?>
													<tr>
														<!-- <td><?= $dtl->id; ?></td> -->
														<td class="py-3"><a href="?d=<?= $dtl->rekaman_id . "&sub=" . $dtl->id ?>" class="text-dark h4 text-hover-primary"><i class="fa fa-folder text-success mr-2"></i><?= $dtl->rekaman_detail_name; ?></a></td>
									...
									<?php else : ?>
										<div class=" h-100 w-100 ">
												<div class="">
													<div class="d-flex justify-content-between align-items-center">
															<h3 for="" class="fw-bold"><i class="fa fa-list mr-2"></i>File Rekaman</h3>
															<div class="tools pr-3">
																<button type="button" class="btn btn-success btn-sm upload-file" data-rekaman_detail_id="<?= $sub; ?>" data-cat="PRE" title="Upload New File"><i class="fa fa-upload"></i> Upload File</button>
															</div>
														</div>
													<hr class="my-1">
													<table class="table py-0 table-sm table-hover datatable">
														...
														<tbody>
															<?php if (isset($details_data['PRE'])) : foreach ($details_data['PRE'] as $dtDtl) : ?>
																	<tr>
																		<td class="cursor-pointer"><i class="fa fa-file-alt mr-2 text-primary"></i><?= $dtDtl->rekaman_detail_data_name; ?></td>
																		<td class="text-center">
																			<button type="button" class="btn btn-xs btn-icon btn-info view-file" data-rekaman_detail_id="<?= $sub; ?>" data-cat="PRE" data-id="<?= $dtDtl->id; ?>"><i class="fa fa-eye"></i></button>
																			<button type="button" class="btn btn-xs btn-icon btn-warning edit-file" data-doc="<?= $dtDtl->document; ?>" data-rekaman_detail_id="<?= $sub; ?>" data-cat="PRE" data-id="<?= $dtDtl->id; ?>"><i class="fa fa-edit"></i></button>
																			<button type="button" class="btn btn-xs btn-icon btn-danger delete-file" data-doc="<?= $dtDtl->document; ?>" data-rekaman_detail_id="<?= $sub; ?>" data-cat="PRE" data-id="<?= $dtDtl->id; ?>"><i class="fa fa-trash"></i></button>
																		</td>
																	</tr>
															<?php endforeach;
															endif; ?>
...
<script>
	...
	$(document).on('click', '.add-sub-directory', function() {
		const rekaman_id = $(this).data('id')

		$('#modalId .modal-title').text('Add Sub Directory')
		$('#modalId').modal('show')
		$('#modalId .modal-body').html(`
		<label for="">Directory Name</label>
		<input type="hidden" id="rekaman_id" class="form-control" value="` + rekaman_id + `">
	...
	$(document).on('click', '.edit-sub-folder', function() {
		...
		$.getJSON(siteurl + active_controller + 'edit_sub_dir/' + id, function(data) {
			...
			<input type="hidden" id="rekaman_id" class="form-control" value="` + data.data.rekaman_id + `">
	...
	$(document).on('click', '.save-sub-folder', function() {
		...
		const rekaman_id = $('#rekaman_id').val();
	...
	$(document).on('click', '.upload-file', function() {
		...
		const rekaman_detail_id = $(this).data('rekaman_detail_id')
		$('#modalId .modal-title').text('Upload Files')
		$('#modalId').modal('show')
		$('#modalId .modal-dialog').addClass('modal-lg')
		$('#modalId .modal-body').html(`
		<form id="form-header" class="mb-4">
			<input type="hidden" name="rekaman_detail_id" class="form-control mb-3" placeholder="Document Name" value="` + rekaman_detail_id + `" aria-describedby="helpId">
