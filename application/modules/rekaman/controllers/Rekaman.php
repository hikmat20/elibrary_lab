 ...
 */
	...
	public function index()
	{
		if (!isset($_GET['d']) || $_GET['d'] == '') {
			redirect('rekaman/?d=0');
		}

		$ArrDetail 				= '';
		$selected 				= $details = $details_data = $breadcumb = $sub = '';
		$dirs 	  				= $this->db->get_where('rekaman', ['status' => '1', 'company_id' => $this->company])->result();
		if (isset($_GET['d']) && ($_GET['d'])) {
			$selected 			= $_GET['d'];
			$details 	  		= $this->db->get_where('view_rekaman_details', ['rekaman_id' => $selected, 'status' => '1', 'company_id' => $this->company])->result();
			$rekaman 			= $this->db->get_where('rekaman', ['id' => $selected, 'company_id' => $this->company])->row();
			$breadcumb 			= [($rekaman) ? $rekaman->name : ''];
		}

		if ((isset($_GET['d']) && ($_GET['d'])) && (isset($_GET['sub']) && ($_GET['sub']))) {
			$selected 			= $_GET['d'];
			$sub 				= $_GET['sub'];
			$details 	  		= $this->db->get_where('view_rekaman_details', ['id' => $sub, 'status' => '1', 'company_id' => $this->company])->row();
			$details_data 	  	= $this->db->get_where('view_rekaman_detail_data', ['rekaman_detail_id' => $sub, 'status' => '1', 'company_id' => $this->company])->result();
			$breadcumb 			= ($details) ? [$details->rekaman_name, $details->rekaman_detail_name] : '';
	...
	public function save_dir()
	{
		...
		if (!isset($post['id'])) {
			$name 			= $post['name'];
			$is_exist 		= $this->db->get_where('rekaman', ['name' => $name])->num_rows();
			...
			$this->db->update('rekaman', $data, ['id' => $post['id']]);
		}
	...
	public function delete_dir()
	{
		...
		$check_child = $this->db->get_where('rekaman_details', ['rekaman_id' => $id])->num_rows();

		$this->db->trans_begin();

		if ($check_child > 0) {
			$data_detail = $this->db->get_where('rekaman_details', ['rekaman_id' => $id,], ['rekaman_id' => $id])->result();

			$this->db->update('rekaman_details', ['status' => '0'], ['rekaman_id' => $id]);
			foreach ($data_detail as $dtl) {
				$detail_data = $this->db->get_where('rekaman_detail_data', ['rekaman_detail_id' => $dtl->id])->result();

				if (count($detail_data) > 0) {
					$this->db->update('rekaman_detail_data', ['status' => '0'], ['rekaman_detail_id' => $dtl->id]);
				}
			}
		}

		$this->db->update('rekaman', ['status' => '0'], ['id' => $id]);
	...
	{
		if ($id) {
			$data = $this->db->get_where('rekaman_details', ['id' => $id])->row();
			echo json_encode(['data' => $data]);
		}
	}
	...
	public function save_sub_dir()
	{
		...
		if (!isset($post['id'])) {
			$name 			= $post['name'];
			$rekaman_id 		= $post['rekaman_id'];
			$is_exist 		= $this->db->get_where('rekaman_details', ['name' => $name])->num_rows();
			...
			$data = [
				'name' => strtoupper($name),
				'company_id' 	=> $this->company,
				'rekaman_id' 	=> $rekaman_id,
				'created_by' 	=> $this->auth->user_id(),
				'created_at' 	=> date('Y-m-d H:i:s'),
			];

			$this->db->insert('rekaman_details', $data);
		} else {
			$name 			= $post['name'];
			$data = [
				'name' => strtoupper($name),
			...
			$this->db->update('rekaman_details', $data, ['id' => $post['id']]);
		}
	...
	{
		$id 		 = $this->input->post('id');
		$check_child = $this->db->get_where('rekaman_detail_data', ['rekaman_detail_id' => $id])->result();

		$this->db->trans_begin();

		if (count($check_child) > 0) {
			$this->db->update('rekaman_detail_data', ['status' => '0'], ['rekaman_detail_id' => $id]);
		}

		$this->db->update('rekaman_details', ['status' => '0'], ['id' => $id]);
	...
	public function save_document()
	{
		...
		try {
			...
			if ($nama_file) {
				$this->db->trans_begin();
				$check = $this->db->get_where('rekaman_detail_data', ['id' => $id])->num_rows();
				...
				if (intval($check) == '0') {
					...
					$this->db->insert('rekaman_detail_data', $data);
				} else {
					...
					$this->db->update('rekaman_detail_data', $data, ['id' => $id]);
				}
			}
		} catch (Exception $e) {
			$this->db->trans_rollback();
	...
	public function edit_file($id)
	{
		$data = [];
		if ($id) {
			$data = $this->db->get_where('rekaman_detail_data', ['id' => $id])->row();
	...
	public function delete_file()
	{
		$id = $this->input->post('id');
		if ($id) {
			$this->db->update('rekaman_detail_data', ['status' => '0'], ['id' => $id]);
